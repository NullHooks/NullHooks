...
bool hooks::initialize() {
	...
	if (MH_CreateHook(present, &present::hook, reinterpret_cast<void**>(&present::original)) != MH_OK)
		throw std::runtime_error("failed to initialize present hook.");
	custom_helpers::state_to_console_color("Hooks", "present hook initialized!");
    
	if (MH_CreateHook(reset, &reset::hook, reinterpret_cast<void**>(&reset::original)) != MH_OK)
		throw std::runtime_error("failed to initialize reset hook.");
	custom_helpers::state_to_console_color("Hooks", "reset hook initialized!");
