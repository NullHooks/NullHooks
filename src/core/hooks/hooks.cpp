...
#define MAKE_HOOK(target, new_function, original, function_name) if ( MH_CreateHook( target, &new_function, reinterpret_cast< void** >( &original ) ) != MH_OK ) throw std::runtime_error( std::string("failed to initialize ").append(function_name) ); else helpers::console::state_to_console_color( "Hooks", std::string(function_name).append(" initialized!").c_str() ); // todo - use std::vformat (im just lazy)
