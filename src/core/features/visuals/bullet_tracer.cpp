...
void visuals::bullet_tracer(i_game_event* event) {
	...
	if (is_friend) {
		if (!variables::misc_visuals::bulletracer_team_options.at(0).state && entity == csgo::local_player)		 return;	// We shot but we dont care
		else if (!variables::misc_visuals::bulletracer_team_options.at(1).state && entity != csgo::local_player) return;	// An ally shot but we dont care
	} else if (!variables::misc_visuals::bulletracer_team_options.at(2).state)									 return;	// An enemy shot but we dont care
	...
	if (variables::misc_visuals::bulletracer_draw_options.at(1).state)											// Draw a box at the hit point
