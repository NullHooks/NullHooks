...
void backtrack::run(c_usercmd* cmd) noexcept {
	...
	if (best_target) {			// We found a good target
		if (records[besst_target_index].size() <= 3) return;	// Check if we have less than 3 valid records. Note: Not entirely sure why it does this you can just check if the deque is empty.
